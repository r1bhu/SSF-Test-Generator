...
int main(int argc, char* argv[])
{
	...
	vector<Gate*> unresolvedGatesCopy = unresolvedGates;
	...
	for (int iter = 0; iter < atoi(argv[2]); iter++)
	{
		...
		for (int i = 0; i < inpNodesList.size(); i++)
		{
			inpStr += to_string(rand() % 2);

		}

		while (randomStrings.find(inpStr) != randomStrings.end())
		{
			inpStr = "";
			for (int i = 0; i < inpNodesList.size(); i++)
			{
		...
		randomStrings.insert(inpStr);

		/* Clear idFaultMapping */
		idFaultMapping.clear();
		...
		int coverage = finalFaultList.size();
		if (coverage == 2 * nodeList.size())
		{
			break;
		}

		cout << randomStrings.size() << " " << (float)finalFaultList.size() * 50 / (float)nodeList.size() << endl;

		unresolvedGates = unresolvedGatesCopy;
